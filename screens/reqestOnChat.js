...
import { db, doc,auth, collection, onSnapshot, query, where,updateDoc,getDocs} from "../firebase";
      ...
      const unsubscribe = onSnapshot(queryChats, (querySnapshot) => {
        ...
        setuserobj(
          querySnapshot?.docs?.map((doc) => {
            ...
            return { email: doc.data().email, userName: doc?.data()?.userName,uid:doc?.data().uid };
          })
        );
      });
      return unsubscribe;
